...
>
> **Update 4:** guest meals are now billed to whichever person hosted them instead of being an unattributed shared cost — see item 3 below. The guest counter moved from one shared control per meal-slot to one under each person's own row. Verified live end-to-end: added guests under a specific person, confirmed the "N meals" badge and Recent Days table update correctly, added a real expense, and confirmed the report page billed the host's meal cost for their own meals *plus* their guests while the other member's cost stayed based only on their own meals (with balances still netting to exactly zero).
...
3. **Guest meals are billed to whoever hosted them.** Each person has their own guest-meal counter (shown right under their Ate/Not-eaten toggle on the Meals page), instead of one shared count per day/meal-slot. A guest counts toward the shared meal-cost divisor the same as anyone else's meal, but the resulting cost is added to *that person's* bill, not split across everyone — so if Taufiq brings a guest to dinner, Taufiq pays for it, not the household in general. (Earlier versions of this app treated guest meals as an unattributed shared cost with no host — that's what changed here. Migration `20260907190343_add_guest_meal_host` adds the required `hostUserId` column; any pre-existing guest-meal rows with a real count get attributed to the admin account since the old data had no host recorded, and empty placeholder rows are dropped.) Balances always net to zero regardless of guest meals now, since every meal — guest or not — is billed to exactly one person (verified in `tests/calculations.test.ts`).
4. **Permissions** (enforced server-side in every Server Action, not just hidden in the UI):
   - **Member (User role):** view dashboard/report/history, toggle **their own** meals, add guest-meal counts **under their own name**, **add** expenses.
...
11. **No password reset on login** — there's no forgot-password flow (no SMTP/Resend/SendGrid credentials were available to configure emailed resets). If someone forgets their password, an admin resets it for them from Admin → Users. If you later want a real emailed reset link, that's a bigger addition — give me email-provider credentials and I can wire it up.
